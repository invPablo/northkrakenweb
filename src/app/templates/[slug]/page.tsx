...
export default async function TemplateDetailPage({
  ...
  return (
    <div className="bg-background-dark text-foreground-light min-h-screen font-sans">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 bg-accent text-white font-semibold px-7 py-3.5 rounded-full hover:brightness-110 transition"
            >
            ...
            {tpl.demoUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 border border-black/15 px-7 py-3.5 rounded-full text-zinc-700 hover:border-black/30 transition"
              >
      ...
      {/* PREVIEW GALLERY */}
      <section className="px-6 lg:px-20 py-16 lg:py-24 border-t border-black/10">
        <span className="tech-text text-accent">Preview</span>
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {tpl.images
            ? tpl.images.map((src) => (
                <div key={src} className="aspect-[4/3] rounded-2xl overflow-hidden relative border border-black/10">
            ...
            : ["01", "02", "03"].map((n) => (
                <div key={n} className="aspect-[4/3] card-surface rounded-2xl flex items-center justify-center">
                  <span className="font-display uppercase text-4xl text-zinc-300">{n}</span>
                </div>
              ))}
        </div>
        {!tpl.images && (
      ...
      {/* FEATURES + PAGES */}
      <section className="px-6 lg:px-20 py-16 lg:py-24 grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-black/10">
        <div>
      ...
      {/* CTA */}
      <section className="px-6 lg:px-20 py-20 lg:py-28 text-center border-t border-black/10">
